...
if page == "home":
    ...
    st.markdown(
        """
**UlasAnalisa** adalah website untuk menganalisis sentimen ulasan aplikasi di Google Play Store secara otomatis dan menyajikannya dalam tabel yang mudah dipahami. Hasil sentimen juga dapat diunduh dalam format **.csv**.
"""
    )
    ...
    with c1:
        st.image("static/2.png", width=MOBILE_W)
    with c2:
        st.image("static/3.png", width=MOBILE_W)
    with c3:
        st.image("static/4.png", width=MOBILE_W)
...
# HALAMAN PREDIKSI
    ...
    if run:
        ...
        with st.spinner("Mengubah fitur (TF-IDF) dan memprediksi"):
            ...
            if model_name == "SVM (RBF)":
                ...
                t2 = df.copy()
                t2["pred"] = y_rf
                t2["pred_label"] = t2["pred"].map({1: "Positive", 0: "Negative"})
            ...
            if st.session_state.is_combo:
                ...
                dist_rf = (
                    df_rf["pred_label"].value_counts().rename_axis("sentiment").reset_index(name="count")
                )
                out2 = io.BytesIO()
    ...
    if st.session_state.results and page == "prediksi":
        ...
        for c, (name, dfm) in zip(cols, items):
            with c:
                ...
                LEGEND_POS = alt.Legend(
                    title=None, orient="top", direction="horizontal", labelLimit=1000, symbolSize=120, padding=8
                )
                ...
                labels = (
                    alt.Chart(dist)
                    .mark_text(dy=-6)
                    .encode(x="Sentiment:N", y="Count:Q", text="Count:Q", color=alt.value("black"))
                )
                ...
                LEGEND_POS = alt.Legend(
                    title=None, orient="bottom", direction="horizontal", labelLimit=160, symbolSize=120, padding=8
                )
                ...
                labels_outline = (
                    ...
                    .encode(theta=alt.Theta("Count:Q", stack=True), text=alt.Text("pct_txt:N"))
                )
                labels_fill = (
                    base.transform_filter(alt.datum.pct >= 0.03)
                    .mark_text(
                ...
                pie = (arc + labels_outline + labels_fill).properties(
                ...
                ax.set_title(f"Proporsi Sentimen – {name}")
                ax.axis("equal")
        ...
        # Build CM
        pairs = []
        for model_name_i, dfm in st.session_state.results.items():
            tmp = dfm.copy()
            # true label dari rating bintang: >=4 -> POS(1), <4 -> NEG(0)
            ...
            y_pred = tmp["pred"].astype(int).values
            cm = confusion_matrix(y_true, y_pred, labels=[1, 0])
            pairs.append((model_name_i, cm))

        if not pairs:
            st.info("Tidak ada data untuk membuat Confusion Matrix.")
        else:
            cols = st.columns(2, gap="small")
            for i, (nm, cm) in enumerate(pairs):
                with cols[i % 2]:
                    fig_cm = plot_cm_v2(cm, nm)
                    st.pyplot(fig_cm, use_container_width=True)
                    # simpan gambar CM
                    all_fig_images.append((f"{nm}_confusion_matrix.png", fig_to_png_bytes(fig_cm)))

                # Keterangan & metrik cepat
...
"""
        ...
        if metrics:
            ...
            # Urutkan berdasar F1 lalu Accuracy
            ...
            # Model terbaik per metrik (opsional)
        ...
        with pd.ExcelWriter(out, engine="xlsxwriter") as w:
            ...
            for fname, png in all_fig_images:
                ...
                ws.insert_image(
                    ...
                    col,
                    fname,
                    {"image_data": io.BytesIO(png), "x_scale": 0.9, "y_scale": 0.9},
                )
                row += 22
